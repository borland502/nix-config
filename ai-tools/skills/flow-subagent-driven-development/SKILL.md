...
digraph when_to_use {
    ...
    "flow-subagent-driven-development" [shape=box];
    "flow-executing-plans" [shape=box];
    ...
    "Stay in this session?" -> "flow-subagent-driven-development" [label="yes"];
    "Stay in this session?" -> "flow-executing-plans" [label="no - parallel session"];
}
```
...
- Final whole-branch review: use nix-config-tools:git-request-review's [code-reviewer.md](../git-request-review/code-reviewer.md)
...
- **nix-config-tools:git-request-review** - Code review template for the final whole-branch review
