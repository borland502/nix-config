...
- Bash command + output is logged automatically by the `log-bash` PostToolUse
  hook (`~/.cache/claude/session_<id>.log`, capped at 30000 chars/field to
...
- `~/.config/claude/` is harness state, not an artifact directory: never write
  ...
  and `cache-scan` lifecycle only covers `~/.cache/claude`). Put them in
  `~/.cache/claude` under a `<date>-<topic>` name, and reference them by that
...
- Download attachments and fetched files to named files under
  `~/.cache/claude/`; do not stream bytes through shell logs.
...
- For shell commands with JSON payloads, inline scripts, or heavy quoting,
  write a short script file under ~/.cache/claude and execute it instead of
...
nix-config repo (`claude` substituted per agent) and deployed read-only by
