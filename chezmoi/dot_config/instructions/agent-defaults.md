...
- Preserve repositories: unless explicitly requested as tracked deliverables,
  ...
  payloads, and logs to `~/.cache/@@AGENT@@` or an existing ignored operational
...
- `~/.config/@@AGENT@@/` is harness state, not an artifact directory: never write
  ...
  and `cache-scan` lifecycle only covers `~/.cache/@@AGENT@@`). Put them in
  `~/.cache/@@AGENT@@` under a `<date>-<topic>` name, and reference them by that
...
- Download attachments and fetched files to named files under
  `~/.cache/@@AGENT@@/`; do not stream bytes through shell logs.
