...
in {
  ...
  environment = {
    ...
    systemPackages = with pkgs; [
      ...
      obsidian
      firefox
      # WhatsApp was dropped entirely on 2026-08-21 (Homebrew cask removed too).
      # If it is ever re-added, use the Homebrew cask, NOT nixpkgs'
      # whatsapp-for-mac: that fetcher pulls a live, per-version vendor URL
      # which WhatsApp stops serving as soon as it ships a new release, so the
      # fixed-output derivation starts 500ing from every mirror and is not
      # substitutable from cache.nixos.org either. That took Darwin CI down
      # four days after the last green run, on nothing but the calendar
      # (pin 2.26.19.17 vs 2.26.31.27 upstream).
      flameshot
    ];
